<script>
  ...
  let totalCreated = createdData.reduce((a, b) => a + b, 0);
  let totalCompleted = completedData.reduce((a, b) => a + b, 0);
  
  // Función para convertir números a porcentajes
    ...
    series: [
      {
        ...
        dataLabels: {
          ...
          style: {
            ...
            fontSize: '10px',  // Reducir el tamaño de la fuente de los porcentajes
          },
          ...
          formatter: function() {
            return toPercentage(this.y, totalCreated + totalCompleted);  // Muestra el porcentaje
          }
        }
      }
    ],
  ...
  let lineChartOptions = {
    ...
    series: [
      { 
        name: 'Creadas', 
        data: createdData.map(value => toPercentage(value, totalCreated + totalCompleted)) 
      },
      { 
        name: 'Completadas', 
        data: completedData.map(value => toPercentage(value, totalCreated + totalCompleted)) 
      }
    ],
    credits: { enabled: false }
  };
